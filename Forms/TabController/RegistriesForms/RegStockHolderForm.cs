...
{
    public partial class RegStockHolderForm : Form
    {
        public event EventHandler RCRDUpdated;
        string ID = null,  Name = null, FName = null, Phone = null, Address = null, dstPath = null;
        bool isRegd;
        public RegStockHolderForm(bool isReg = false, DataGridViewRow row = null)
        {
            InitializeComponent();
            ...
            if (row != null)
            {
                ...
                Name = tmp;
                tmp = row.Cells["FName"].Value.ToString();
        ...
        private void OpenImageBTN_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection con = Configurations.DataBaseConnection.ConnectToSqlServer(true, true, true, true))
                {
                    ...
                    {
                        string imageName;
                        object tmp = new SqlCommand("SELECT MAX (ID) from StockHolders", con).ExecuteScalar();
                        ...
                        if (!Directory.Exists(Directory.GetCurrentDirectory() + Configurations.ConfigurationKeys.ImagesDataDirecotry + Configurations.ConfigurationKeys.STOCKHOLDERTABLEIMAGES)) Configurations.ProjectConfigurations.createDirectory(Configurations.ConfigurationKeys.ImagesDataDirecotry + Configurations.ConfigurationKeys.STOCKHOLDERTABLEIMAGES);
                        openFile.FileName = $"{imageName.ToUpper()}";
                        openFile.Filter = "Image Files(*.jpg, *.jpeg, *.png)|*.jpg; *.JPEG; *.png|Icons Files(*.ico)|*.ico|All files(*.*)|*.*";
                        openFile.Title = "Open Image File for the Stockholders!";
                        if (openFile.ShowDialog() == DialogResult.OK)
                        {
                            srcpath = openFile.FileName;
                            dstPath = Directory.GetCurrentDirectory() + Configurations.ConfigurationKeys.ImagesDataDirecotry + Configurations.ConfigurationKeys.STOCKHOLDERTABLEIMAGES + "\\" + imageName + Path.GetExtension(openFile.FileName);
                            try
                            {
        ...
        {
            if (Name != null && FName != null && Phone != null && Address != null && dstPath != null)
            {
                try
                {
                    ...
                    using (SqlConnection con = Configurations.DataBaseConnection.ConnectToSqlServer(true, true, true, true))
                    {
                        try
                        {
                            new SqlCommand($"INSERT INTO StockHolders (Name, FName, Phone, Address, Image) VALUES ('{Name}', '{FName}', '{Phone}', '{Address}', '{dstPath}');", con).ExecuteNonQuery();
                            this.NameTXT.Text = this.FNameTXT.Text = this.PhoneTXT.Text = this.AddressTXT.Text = Name = FName = Phone = Address = ID = dstPath = "";
        ...
        private void DeleteRecord_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection con = Configurations.DataBaseConnection.ConnectToSqlServer(true, true, true, true))
                {
                    try
                    {
                        new SqlCommand($"delete from StockHolders where ID = '{ID}';", con).ExecuteNonQuery();
                        this.NameTXT.Text = this.FNameTXT.Text = this.PhoneTXT.Text = this.AddressTXT.Text = Name = FName = Phone = Address = ID = dstPath = "";
            ...
            try
            {
                using (SqlConnection con = Configurations.DataBaseConnection.ConnectToSqlServer(true, true, true, true))
                {
                    try
                    {
                        ...
                        new SqlCommand($"UPDATE StockHolders set {UpdateQuery} where ID = '{ID}'", con).ExecuteNonQuery();
                        this.NameTXT.Text = this.FNameTXT.Text = this.PhoneTXT.Text = this.AddressTXT.Text = Name = FName = Phone = Address = ID = dstPath = "";
